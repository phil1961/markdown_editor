...
const Doc = (() => {
    ...
    function toggleBlock(type) {
        ...
        indices.forEach(i => {
            const block = state.blocks[i];
            if (type === "quote") {
                state.blocks[i] = { type: "quote", blocks: [{
    ...
    function runContaining(pos) {
        ...
        if (block.items) {
            const it = itemAt(block, loc.offset);
    ...
    function splitBlock() {
        ...
        const { left, right } = splitInlinesAt(block.inlines, loc.offset);
        block.inlines = left;
        const newType = /^h[1-6]$/.test(block.type) ? "p" : "p";
        state.blocks.splice(loc.block + 1, 0, { type: newType, inlines: right });
        setSelection(from + 1, from + 1);
    }
    ...
    function deleteBackward() {
        ...
        if (block.items) {
            ...
            if (it.offset === 0 && it.item > 0) {
                const prevLen = inlinesText(block.items[it.item - 1]).length;
            ...
            if (it.offset === 0 && it.item === 0) {
                ...
                if (rest.length) state.blocks.splice(loc.block, 1, para, { type: block.type, items: rest });
                else state.blocks[loc.block] = para;
                setSelection(index(state)[loc.block].from);
                return;
            }
        }
        if (loc.offset === 0 && loc.block > 0) {
            ...
            } else if (prev.items && cur.inlines) {
