...
**Parser suite** loads `dist/editor.cjs`. It pins fences, nested quotes, URL sanitising, tables, and `_snake_case_`. Revert the fence extraction in `src/20-parser.js` (put the ``` replace back after the bold regexes) and the "fences are opaque" group fails. On Windows it also runs `markdown-editor.ps1 -NoLaunch` and checks the injected `%TEMP%` copy.
